...
export function buildSubDimensionPrompt(
  ...
  // 注意：下面的 prompts 对象中包含的是带有占位符的模板字符串
  // 占位符如 ${teacherDoc} 和 ${dialogueText} 是字面量，需要在返回前手动替换
...
`,
  ...
  // 获取原始模板字符串
  const template = prompts[dimensionKey]?.[subDimensionKey];

  if (!template) {
    return "";
  }

  // 手动替换占位符（因为模板字符串中的 ${...} 是字面量，不会自动替换）
  let result = template;
  result = result.replace(/\$\{teacherDoc\}/g, teacherDoc);
  result = result.replace(/\$\{dialogueText\}/g, dialogueText);
  if (workflowConfig) {
    result = result.replace(/\$\{workflowConfig\}/g, workflowConfig);
  }
