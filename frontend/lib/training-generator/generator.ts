...
8. 必须保留原有 flowType 和 edges；如果 flowType="graph"，不得删除、改成线性或丢失分支连线`;
...
function buildPlanningPrompt(
    ...
    segments.push(
        ...
        flowPreference === "linear"
            ...
            : flowPreference === "graph"
                ? "用户本次明确选择：强制非线性规划。你必须输出 flowType=\"graph\"，并根据教师文档规划分支、平行线路、汇合或结束 edges；不得返回线性规划。"
    ...
    segments.push(
        ...
        flowPreference === "linear"
            ...
            : flowPreference === "graph"
                ? "8. 用户强制非线性：flowType 必须是 \"graph\"，edges 必须覆盖入口分流、各分支汇合以及结束路径；同一 fromModuleId 下的 condition 不得重复。"
                : "8. 如果教师文档或用户修改意见要求多条平行线路、分支跳转、选择不同案例后进入不同模块，必须输出 flowType=\"graph\" 并补全 edges；不要把分支强行串成线性顺序。",
        "9. graph 模式的 condition 应使用 NEXT_TO_* 这类唯一关键词；linear 模式不得输出 edges。",
        "10. 只输出严格合法的 JSON。"
    );
    ...
    return [
        ...
        flowType === "graph"
            ? [
                ...
                "5. 注入时会按 `## 🔀 非线性跳转关系` 的 edges 创建连线，而不是按阶段顺序串联。",
            ].join("\n")
            ...
            messages: [
                { role: "system", content: SCRIPT_STRUCTURE_REPAIR_SYSTEM_PROMPT },
                {
                    ...
                    content: [
                        ...
                        "8. 如果模块规划 flowType=graph，必须保留或补全 `## 🔀 非线性跳转关系` JSON 区块，edges 的 condition 要与阶段提示词中的纯净跳转关键词一致。",
