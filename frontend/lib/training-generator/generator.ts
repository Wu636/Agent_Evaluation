...
- suggestedRounds 必须是大于等于 1 的整数，根据模块实际复杂度合理设置；模块不再限制轮数上限
- 不得仅因为 suggestedRounds 较大而拆分模块；只有任务阶段本身不同、流程分支不同或需要切换智能体角色时才拆分模块`;
...
4. suggestedRounds 必须是大于等于 1 的整数，根据模块实际复杂度合理设置，不限制轮数上限
...
5. recommendedMode 和 moduleType 只能是 general|sequential|roleplay|summary
6. suggestedRounds 必须是大于等于 1 的整数，根据模块实际复杂度合理设置，不限制轮数上限
...
function buildPlanningPrompt(
    ...
    segments.push(
        ...
        "5. 学生一次发言 + AI 一次回复记为 1 轮；suggestedRounds 按模块实际复杂度合理设置，不限制轮数上限。",
        "6. 不得仅因为 suggestedRounds 较大而拆分模块；只有任务阶段本身不同、流程分支不同或需要切换智能体角色时才拆分模块。",
