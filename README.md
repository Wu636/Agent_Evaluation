...
### 🚀 流式响应改造 (Latest)
- **彻底解决 Vercel 60s 超时限制**：
  - 后端改用 SSE (Server-Sent Events) 流式响应
  - LLM 调用改为流式模式，逐 token 输出
  - 只要 LLM 持续输出，Vercel 就不会超时（即使总耗时超过 3 分钟）
- **动态并发池**：替换原有 Promise.all 批处理，避免队头阻塞

### 🎨 UI/UX 全面升级
1. **报告详情页重构**:
   - 右侧栏 **Sticky** 吸附，长报告阅读更从容
   - 移除无用的"综合分析"，替换为 **"优先改进建议"** (自动提取 Top 5 严重问题)
   - 侧边栏子维度列表改为 **折叠式 Accordion**
   - 点击子维度可 **精确滚动** 到对应卡片位置
   - **严重问题汇总** 按维度分组折叠展示
2. **视觉优化**: 
   - 统一使用 **Indigo (靛青)** 高级配色
