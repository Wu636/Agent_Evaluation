...
training-script-generator-v2 支持三种核心任务类型，需要根据任务特征选择对应的提示词结构。
...
- 医学护理：床旁护理礼仪（规范vs不规范）→ 共情沟通（到位vs不足）→ 专业解释
...
**定义**：在同一个训练任务中，智能体在不同阶段扮演不同角色，通常用于"理论学习+实操演练"的组合场景。

**特征**：
- 分阶段角色切换（如：带教老师 → 患者/客户/用户）
- 第一阶段：知识考核（顺序检查型或累积检查型）
- 第二阶段：实操模拟（顺序检查型）
- 需要明确的角色切换节点

**典型场景**：
- 医学护理：理论诊断（带教老师）→ 床旁护理（患者）
- 客户服务：理论考核（主管）→ 客户沟通（客户）
- 教学实训：教案分析（导师）→ 课堂模拟（学生）
...
| 判断维度 | 顺序检查型 | 累积检查型 | 双角色混合型 |
|---------|-----------|-----------|-------------|
| **对错判断** | 有明确的对错标准 | 没有对错，只有齐全vs缺失 | 分阶段：第一阶段累积/顺序，第二阶段顺序 |
| **顺序要求** | 通常按固定顺序 | 顺序不固定 | 阶段顺序固定，角色切换明确 |
| **判定依据** | 当前回答是否正确 | 对话历史中是否提及所有要素 | 综合使用前两种判定方式 |
| **核心机制** | 正确/错误分支 | Step 0（上下文回溯）+ 缺失判定 | 角色切换 + 前两种机制的组合 |
| **典型问法** | "这样做对吗？" | "你列出了哪些XX？" | "先分析病例，再进行床旁护理" |
...
| **7轮以上** | 深度交互，复杂任务 | 故障排查、深度分析、多轮共情 | 多层次追问，按实际需要设置 |

### 轮次设定技巧

1. **起始阶段通常0-3轮**：快速建立场景，不宜过长
2. **核心阶段通常5轮以上**：按实际复杂度留出足够空间让学生犯错、修正、学习，不限制轮数上限
...
**作用**：提供智能体的第一句话，启动对话。这是 training-script-generator-v2 的核心创新功能，自动生成每个阶段的开场白。
...
- 抛出第一个引导或问题
- 使用引号标注实际的对话内容
- **医学/专业场景**：开场白可以较长，包含完整的病例/背景信息（100-300字）
- **日常对话场景**：开场白简短，20-50字
...
```markdown
# Opening Line
"咱们今天的分析病例是34岁的林晚，市场总监，前天入院做保乳术前准备。她是激素依赖型乳腺癌Ⅰ期，明天做'右侧乳腺病灶扩大切除术+前哨淋巴结活检'，术后要吃5年他莫昔芬。我给你念下昨天的护理记录：①7:30备皮时，她摸着右侧乳房说'护士，切完这块是不是就凹进去了？以后穿衣服肯定一边高一边低'，说完红了眼眶..."
```
...
**说明**：
- 平台会自动将"开场白"字段的内容作为该阶段的第一句话输出
- 提示词中的 Opening Line 用于提醒智能体"你已经说过这句话了"，避免重复输出
- 这样设计可以确保每个阶段都有明确的引导，学生不会遇到冷启动问题
...
2. **判定回答部分正确**:
   ...
   - 回复示例：
     ...
     - 若遗漏预感性悲哀的表现："预感性悲哀的表现还要加上'拒绝试穿康复内衣'和'食欲下降'哦～"
     - 若诊断名称不规范："诊断名称要用规范的心理护理诊断术语，比如'预感性悲哀''焦虑'～"
```

**说明**：
- 医学场景需要提供**多个回复示例**，覆盖不同的错误类型
- 每个示例都要**具体指出问题**（如"遗漏XX表现"、"诊断名称不规范"）
- 语气要**专业但温和**（使用"～"等柔和符号）
...
- **跳转纯净性**：当满足跳转条件时，**仅输出**跳转关键词 `NEXT_TO_XXX`，严禁输出任何其他文字
- 单次回复字数限制：[N-M字]
```

**字数限制指南**：
- 快速交互场景：20-50字（如简单确认、选择）
- 中等深度场景：50-100字（如引导性对话）
- 深度交互场景：80-150字（如专业解释、情感共鸣）
- 医学病例介绍：可放宽到200-300字（仅限0轮阶段的开场白）

**跳转纯净性的重要性**：
```markdown
# Response Constraints
- **跳转纯净性**：当学生回答准确完整时，**仅输出**跳转关键词 `NEXT_TO_GOAL`，严禁输出任何其他文字
```

**说明**：
- ❌ 错误：输出 "很好！你说得对，我们进入下一阶段。NEXT_TO_GOAL"
- ✅ 正确：仅输出 "NEXT_TO_GOAL"
- 跳转纯净性确保系统能准确识别跳转指令，避免识别失败
...
- 根据学生回答的具体情况灵活给予反馈
- 反馈要具体指出哪里需要修正
- **跳转纯净性**：当学生回答准确完整时，**仅输出**跳转关键词 `NEXT_TO_MEASURES`，严禁输出任何其他文字
- 单次回复字数：50-150字
```
...
**原理**：在提示词中提供多个具体的回复示例，覆盖不同的错误类型，确保智能体给出精准的引导。
...
- [ ] 错误回答的反馈是否能让学生意识到问题？
- [ ] 正确回答的判定条件是否明确？
- [ ] **医学场景**：是否提供了具体的回复示例，覆盖常见错误类型？
- [ ] **医学场景**：回复示例是否使用了专业术语？语气是否温和？
