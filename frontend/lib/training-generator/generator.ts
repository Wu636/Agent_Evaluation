...
import { extractScriptStructure, extractStageNumberFromHeading, TRAINING_SCRIPT_COMPLETE_MARKER } from "./script-tools";
...
如果任务存在分支、平行线路、按用户回复进入不同模块或分支后汇总，必须生成非线性图结构，并在阶段后输出 \`## 🔀 非线性跳转关系\` JSON 区块；普通任务保持线性结构。
完整剧本生成结束后，最后一个非空行必须输出：${TRAINING_SCRIPT_COMPLETE_MARKER}。如果尚未输出到文档末尾，禁止提前输出该标志。
不要只输出 JSON，不要做评分，不要输出与剧本配置无关的内容。`;
...
2. JSON 结构必须是：
{
  ...
  "edges": [
    {
      "id": "edge_1",
      "fromModuleId": "module_1",
      "toModuleId": "module_2 或 __END__",
      "condition": "跳转关键词",
      "conditionDescription": "触发该跳转的条件说明",
      "transitionPrompt": "该条连线专用的衔接提示词",
      "isDefault": 1
    }
  ],
  "notes": ["补充说明1", "补充说明2"]
...
- 同一训练任务可以混合多种模块类型
- 如果任务是普通顺序推进，flowType 输出 "linear"，edges 输出空数组 []
- 如果教师文档存在“多案例并行、用户选择不同线路、按回答分流、多个平行任务、分支后汇总”等非线性结构，flowType 输出 "graph"，edges 必须列出所有分支连线与汇合连线
- graph 模式下每条边必须使用唯一且可识别的 condition，例如 NEXT_TO_CASE_A、NEXT_TO_CASE_B、NEXT_TO_SUMMARY_FROM_A；同一 fromModuleId 下不能重复 condition
- graph 模式下每条边必须包含 isDefault；同一 fromModuleId 下只能有一条边 isDefault=1，其他出口必须为 0
- graph 模式下终止连线的 toModuleId 使用 "__END__"，且每条到不同线路的 transitionPrompt 要体现该线路语境，不能共用泛化提示词
...
9. 若原稿存在分支、并行线路或汇合关系，必须保留为 flowType="graph" 并补全 edges；普通顺序流程使用 flowType="linear" 和 edges=[]
10. graph 模式下每条边必须包含 isDefault；同一 fromModuleId 下只能有一条边 isDefault=1，其他出口必须为 0`;
...
8. 必须保留原有 flowType 和 edges；如果 flowType="graph"，不得删除、改成线性或丢失分支连线，并保持同一 fromModuleId 下最多一条 isDefault=1`;
...
function normalizeFlowType(value: unknown, hasEdges: boolean): TrainingFlowType {
    const cleaned = String(value || "").trim().toLowerCase();
    if (cleaned === "graph" || cleaned === "nonlinear" || cleaned === "branch") return "graph";
    if (cleaned === "linear") return "linear";
    return hasEdges ? "graph" : "linear";
}

function normalizePlanEdgeDefault(value: unknown): number | undefined {
...
function normalizePlanEdges(rawEdges: unknown): ScriptFlowEdge[] {
    const items = Array.isArray(rawEdges) ? rawEdges as Array<Record<string, unknown>> : [];

    return items.map((edge, index) => {
        const fromModuleId = String(edge.fromModuleId ?? edge.from ?? edge.source ?? edge.sourceModuleId ?? "").trim();
        const toModuleId = String(edge.toModuleId ?? edge.to ?? edge.target ?? edge.targetModuleId ?? "").trim();
        const isDefault = normalizePlanEdgeDefault(edge.isDefault ?? edge.default ?? edge.is_default);
        return {
            id: String(edge.id || makeEdgeId(index)),
            fromModuleId,
            toModuleId,
            condition: String(edge.condition ?? edge.flowCondition ?? edge.keyword ?? "").trim(),
            conditionDescription: String(edge.conditionDescription ?? edge.description ?? edge.when ?? "").trim(),
            transitionPrompt: String(edge.transitionPrompt ?? edge.transition ?? edge.prompt ?? "").trim(),
            ...(isDefault === undefined ? {} : { isDefault }),
        };
    }).filter((edge) => edge.fromModuleId && edge.toModuleId && edge.condition);
}

function isPlanEndNodeId(value: string): boolean {
    const normalized = String(value || "").trim().toLowerCase();
...
function filterValidPlanEdges(edges: ScriptFlowEdge[], moduleIds: Set<string>): ScriptFlowEdge[] {
    return edges.filter((edge) =>
        moduleIds.has(edge.fromModuleId) &&
        (moduleIds.has(edge.toModuleId) || isPlanEndNodeId(edge.toModuleId)) &&
        edge.fromModuleId !== edge.toModuleId &&
        edge.condition.trim()
    );
}

function appendUniqueNote(notes: string[] | undefined, note: string): string[] {
    const existing = notes || [];
    return existing.includes(note) ? existing : [...existing, note];
}

function applyFlowPreferenceToPlan(
    plan: TrainingScriptPlan,
    flowPreference: TrainingFlowPlanningPreference = "auto"
): TrainingScriptPlan {
    if (flowPreference === "linear") {
        return {
            ...plan,
            flowType: "linear",
            edges: [],
            notes: appendUniqueNote(plan.notes, "已按用户选择强制规划为线性顺序流程。"),
        };
    }

    if (flowPreference === "graph") {
        const validEdges = filterValidPlanEdges(plan.edges || [], new Set(plan.modules.map((module) => module.id)));
        return {
            ...plan,
            flowType: "graph",
            edges: validEdges,
            notes: appendUniqueNote(
                ...
                validEdges.length > 0
                    ? "已按用户选择强制规划为非线性分支流程。"
                    : "用户选择了非线性分支流程，但模型未生成有效连线，请在规划编辑器中补充 edges。"
            ),
        };
    }
    ...
    return {
        ...
        edges: plan.flowType === "graph"
            ? filterValidPlanEdges(plan.edges || [], new Set(plan.modules.map((module) => module.id)))
            : [],
        notes: plan.notes || [],
    };
}
...
function normalizePlan(raw: unknown): TrainingScriptPlan {
    ...
    const edges = normalizePlanEdges(obj.edges);
    const flowType = normalizeFlowType(obj.flowType, edges.length > 0);
...
function reconcilePlanWithCurrentPlan(
    ...
    const mergedModuleIds = new Set(mergedModules.map((module) => module.id));
    const validGeneratedEdges = filterValidPlanEdges(generatedPlan.edges || [], mergedModuleIds);
    const useGeneratedGraph = generatedPlan.flowType === "graph" && validGeneratedEdges.length > 0;
    const currentFlowType = currentPlan.flowType || "linear";
    const fallbackCurrentEdges = filterValidPlanEdges(currentPlan.edges || [], mergedModuleIds);
    ...
    return {
        ...
        flowType: useGeneratedGraph ? "graph" : currentFlowType,
        modules: mergedModules,
        edges: useGeneratedGraph
            ? validGeneratedEdges
            : currentFlowType === "graph"
                ? fallbackCurrentEdges
                : [],
...
function enforceRoundLimitBySplittingPlan(plan: TrainingScriptPlan): {
    ...
    const splitReplacementByModuleId = new Map<string, ScriptModulePlan[]>();
    const internalSplitEdges: ScriptFlowEdge[] = [];
    ...
    const modules = plan.modules.flatMap((module, index) => {
        ...
        if (rounds <= MAX_STAGE_ROUNDS) {
            const normalizedModule = { ...module, suggestedRounds: rounds };
            splitReplacementByModuleId.set(module.id, [normalizedModule]);
        ...
        const segmentModules = roundBuckets.map((bucketRounds, segmentIndex) => {
        ...
        splitReplacementByModuleId.set(module.id, segmentModules);
        segmentModules.slice(0, -1).forEach((segmentModule, segmentIndex) => {
            const nextModule = segmentModules[segmentIndex + 1];
            internalSplitEdges.push({
                id: makeRuntimeModuleId(`edge_split_${index + 1}_${segmentIndex + 1}`),
                fromModuleId: segmentModule.id,
                toModuleId: nextModule.id,
                condition: `NEXT_TO_SPLIT_${index + 1}_${segmentIndex + 2}`,
                conditionDescription: `完成「${segmentModule.title}」后进入拆分后的下一段`,
                transitionPrompt: `承接「${segmentModule.title}」的训练结果，自然引导学生进入「${nextModule.title}」。`,
            });
        });
    ...
    if (plan.flowType === "graph") {
        const remappedEdges = edges.map((edge) => {
            const fromSegments = splitReplacementByModuleId.get(edge.fromModuleId);
            const toSegments = splitReplacementByModuleId.get(edge.toModuleId);
            return {
                ...edge,
                fromModuleId: fromSegments?.[fromSegments.length - 1]?.id || edge.fromModuleId,
                toModuleId: isPlanEndNodeId(edge.toModuleId)
                    ? TRAINING_FLOW_END_NODE_ID
                    : toSegments?.[0]?.id || edge.toModuleId,
            };
        });
        edges = filterValidPlanEdges([...remappedEdges, ...internalSplitEdges], new Set(modules.map((module) => module.id)));
    }
    ...
    return {
        plan: {
            ...plan,
            modules,
            edges: plan.flowType === "graph" ? edges : [],
            notes: splitApplied
...
function buildPlanningPrompt(
    ...
    const flowPreference = options?.flowPreference || "auto";
    segments.push(
        "",
        "<flow_planning_preference>",
        flowPreference,
        "</flow_planning_preference>",
        "",
        flowPreference === "linear"
            ? "用户本次明确选择：强制线性规划。你必须输出 flowType=\"linear\"，edges 必须为空数组 []；即使教师文档包含可选线路，也要整理为顺序推进模块。"
            : flowPreference === "graph"
                ? "用户本次明确选择：强制非线性规划。你必须输出 flowType=\"graph\"，并根据教师文档规划分支、平行线路、汇合或结束 edges；每条 edge 必须包含 isDefault，且同一 fromModuleId 下只能一条为 1；不得返回线性规划。"
                : "用户本次选择：自动识别。请根据教师文档判断使用线性流程还是非线性分支流程。"
    );
    ...
    segments.push(
        ...
        flowPreference === "linear"
            ? "8. 用户强制线性：flowType 必须是 \"linear\"，edges 必须是 []。"
            : flowPreference === "graph"
                ? "8. 用户强制非线性：flowType 必须是 \"graph\"，edges 必须覆盖入口分流、各分支汇合以及结束路径；同一 fromModuleId 下的 condition 不得重复，且只能有一条 isDefault=1。"
                : "8. 如果教师文档或用户修改意见要求多条平行线路、分支跳转、选择不同案例后进入不同模块，必须输出 flowType=\"graph\" 并补全 edges；不要把分支强行串成线性顺序。",
        "9. graph 模式的 condition 应使用 NEXT_TO_* 这类唯一关键词；每条 edge 必须包含 isDefault；linear 模式不得输出 edges。",
        "10. 只输出严格合法的 JSON。"
    );
            ...
            messages: [
                { role: "system", content: SCRIPT_PLAN_AUTOFILL_SYSTEM_PROMPT },
                {
                    ...
                    content: [
                        ...
                        "7. 原规划中的 flowType 和 edges 必须保留；如果 flowType=graph，只能补全字段，不得把非线性连线改成线性。",
                    ].join("\n"),
                },
            ],
        }),
...
export function validateTrainingScriptPlan(plan: TrainingScriptPlan): ScriptPlanValidationIssue[] {
    ...
    const flowType = plan.flowType || "linear";
    if (flowType === "graph") {
        const moduleIds = new Set(plan.modules.map((module) => module.id));
        const validEdges = filterValidPlanEdges(plan.edges || [], moduleIds);

        if (!Array.isArray(plan.edges) || plan.edges.length === 0) {
            issues.push({ level: "error", message: "非线性流程必须至少配置一条跳转连线。", field: "edges" });
        }

        (plan.edges || []).forEach((edge, index) => {
            if (!edge.fromModuleId || !moduleIds.has(edge.fromModuleId)) {
                issues.push({ level: "error", message: `连线 ${index + 1} 的起点模块不存在。`, field: "edges" });
            }
            if (!edge.toModuleId || (!moduleIds.has(edge.toModuleId) && !isPlanEndNodeId(edge.toModuleId))) {
                issues.push({ level: "error", message: `连线 ${index + 1} 的终点模块不存在。`, field: "edges" });
            }
            if (!edge.condition.trim()) {
                issues.push({ level: "error", message: `连线 ${index + 1} 缺少跳转关键词。`, field: "edges" });
            }
            if (edge.fromModuleId === edge.toModuleId) {
                issues.push({ level: "error", message: `连线 ${index + 1} 不能从模块跳转回自身。`, field: "edges" });
            }
        });

        const conditionBySource = new Map<string, Set<string>>();
        const defaultCountBySource = new Map<string, number>();
        validEdges.forEach((edge) => {
            const normalizedCondition = edge.condition.trim();
            const existing = conditionBySource.get(edge.fromModuleId) || new Set<string>();
            if (existing.has(normalizedCondition)) {
                issues.push({
                    level: "error",
                    message: `模块「${plan.modules.find((module) => module.id === edge.fromModuleId)?.title || edge.fromModuleId}」存在重复跳转关键词：${normalizedCondition}`,
                    moduleId: edge.fromModuleId,
                    field: "edges",
                });
            }
            existing.add(normalizedCondition);
            conditionBySource.set(edge.fromModuleId, existing);

            if (normalizePlanEdgeDefault(edge.isDefault) === 1) {
                const defaultCount = (defaultCountBySource.get(edge.fromModuleId) || 0) + 1;
                defaultCountBySource.set(edge.fromModuleId, defaultCount);
                if (defaultCount > 1) {
                    issues.push({
                        level: "error",
                        message: `模块「${plan.modules.find((module) => module.id === edge.fromModuleId)?.title || edge.fromModuleId}」存在多条 isDefault=1 的默认出口。`,
                        moduleId: edge.fromModuleId,
                        field: "edges",
                    });
                }
        ...
        const hasEndPath = validEdges.some((edge) => isPlanEndNodeId(edge.toModuleId));
        if (!hasEndPath) {
            issues.push({ level: "warning", message: "非线性流程尚未配置到训练结束的连线，注入时会为无后继模块自动补结束线。", field: "edges" });
        }
    }

    const summaryModules = plan.modules.filter((module) => module.moduleType === "summary");
    if (summaryModules.length > 0 && flowType !== "graph" && plan.modules[plan.modules.length - 1].moduleType !== "summary") {
...
export async function planTrainingScriptModules(
    ...
    parsedPlan = applyFlowPreferenceToPlan(parsedPlan, flowPreference);
...
export async function regenerateTrainingScriptModule(
    ...
    const targetOutgoingEdges = modulePlan.flowType === "graph"
        ? (modulePlan.edges || []).filter((edge) => edge.fromModuleId === targetModule.id)
        : [];
    const nextFlowCondition = targetOutgoingEdges.length > 0
        ? targetOutgoingEdges.map((edge) => edge.condition).join(" | ")
        : targetStageNumber < modulePlan.modules.length
    ...
    const prompt = [
        ...
        targetOutgoingEdges.length > 0
            ? `本阶段非线性出口：\n${targetOutgoingEdges.map((edge) => `- ${edge.condition}：${edge.conditionDescription || "按规划条件跳转"}；transitionPrompt：${edge.transitionPrompt || "无"}`).join("\n")}`
            : "本阶段非线性出口：无",
...
function formatModulePlanForPrompt(plan?: TrainingScriptPlan): string {
    ...
    const edgeLines = flowType === "graph"
        ? (plan.edges || []).map((edge, index) => {
            const fromIndex = plan.modules.findIndex((module) => module.id === edge.fromModuleId);
            const toIndex = plan.modules.findIndex((module) => module.id === edge.toModuleId);
            const fromLabel = fromIndex >= 0 ? `阶段${fromIndex + 1}` : edge.fromModuleId;
            const toLabel = isPlanEndNodeId(edge.toModuleId)
                ? "END"
                : toIndex >= 0
                    ? `阶段${toIndex + 1}`
                    : edge.toModuleId;
            const explicitDefault = normalizePlanEdgeDefault(edge.isDefault);
            const isDefault = explicitDefault === 0 || defaultAssignedBySource.has(edge.fromModuleId) ? 0 : 1;
            if (isDefault === 1) {
                defaultAssignedBySource.add(edge.fromModuleId);
            }
            return [
                `- 连线${index + 1}: ${fromLabel} -> ${toLabel}`,
                `  - condition: ${edge.condition}`,
                `  - 条件说明: ${edge.conditionDescription || "无"}`,
                `  - transitionPrompt: ${edge.transitionPrompt || "无"}`,
                `  - isDefault: ${isDefault}`,
            ].join("\n");
        })
        : [];
    ...
    return [
        ...
        `流程类型: ${flowType === "graph" ? "非线性图结构" : "线性顺序"}`,
        "",
        moduleLines.join("\n\n"),
        flowType === "graph" ? `\n非线性连线:\n${edgeLines.join("\n") || "- 暂无连线"}` : "",
        ...
        flowType === "graph"
            ? [
                "非线性生成要求：",
                "1. 必须在所有阶段之后输出 `## 🔀 非线性跳转关系` 区块，并用 JSON 代码块写出 flowType 与 edges。",
                "2. edges 中的 from/to 必须使用 `阶段1`、`阶段2` 或 `END`；condition 必须与对应阶段提示词中要求模型纯净输出的跳转关键词完全一致。",
                "3. 存在多个出口的阶段，提示词必须写出分支互斥判定：命中不同条件时分别仅输出对应 condition，不能只写一个 NEXT_TO_STAGE。",
                "4. 每条边的 transitionPrompt 必须体现该线路语境；不同分支不要共用同一段泛化衔接提示词。",
                "5. 每条边必须包含 isDefault；同一个 from 下只能一条 isDefault=1，其他出口必须为 0。",
                "6. 注入时会按 `## 🔀 非线性跳转关系` 的 edges 创建连线，而不是按阶段顺序串联。",
            ].join("\n")
            : "线性生成要求：不需要输出 `## 🔀 非线性跳转关系`；按阶段顺序输出 `## 🔄 阶段跳转关系` 即可。",
    ].join("\n");
}
...
必须一直续写到完整文档结束，并将 ${TRAINING_SCRIPT_COMPLETE_MARKER} 作为最后一个非空行。
...
完整剧本修复结束后，最后一个非空行必须输出：${TRAINING_SCRIPT_COMPLETE_MARKER}。
...
export async function continueTrainingScript(
    ...
    const prompt = [
        ...
        partialContent,
        "",
        `请从断点处继续输出后续内容，直到文档完整结束，并以 ${TRAINING_SCRIPT_COMPLETE_MARKER} 作为最后一个非空行。只输出新增续写部分。`,
    ].join("\n");
            ...
            messages: [
                { role: "system", content: SCRIPT_STRUCTURE_REPAIR_SYSTEM_PROMPT },
                {
                    ...
                    content: [
                        ...
                        "8. 如果模块规划 flowType=graph，必须保留或补全 `## 🔀 非线性跳转关系` JSON 区块，edges 的 condition 要与阶段提示词中的纯净跳转关键词一致，并且同一 from 下只能一条 isDefault=1。",
                        `9. 完整文档最后一个非空行必须是 ${TRAINING_SCRIPT_COMPLETE_MARKER}。`,
                    ].join("\n"),
                },
            ],
        }),
